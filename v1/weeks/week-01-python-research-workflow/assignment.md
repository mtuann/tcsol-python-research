# Assignment: Notebook nghiên cứu đầu tiên của tôi

## Mục tiêu

- Kỹ năng Python: chạy notebook, sửa biến, đọc một tệp CSV nhỏ.
- Kỹ năng nghiên cứu: chuyển chủ đề rộng thành câu hỏi nhỏ sẵn sàng cho dữ liệu.
- Kỹ năng writing: viết một research-question memo có thể phát triển thành đoạn mở đầu paper.
- Tool support: viết trong Markdown cell của notebook; chưa cần Word/Zotero hoặc Overleaf ở Tuần 01.

## Dữ liệu

- File: `data/raw/week01_research_tracks.csv`
- Đơn vị quan sát: một hướng project nghiên cứu có thể phát triển.
- Biến chính: `track`, `small_research_question`, `unit_of_observation`, `likely_output`.

## Core Tasks

Nộp **một notebook đã hoàn thành**: `live_coding.ipynb`.

Start here:

1. Mở `live_coding.html` để đọc trước.
2. Mở Colab bằng nút trên trang nếu muốn chạy trong browser.
3. Trong Colab, chọn **File -> Save a copy in Drive**.
4. Chạy từng cell từ trên xuống.
5. Chỉ sửa các cell có ghi **Bạn sửa**.
6. Nộp bản `.ipynb` đã hoàn thành.

Trong notebook:

1. Sửa các biến về hướng nghiên cứu cá nhân.
2. Chạy toàn bộ cell từ đầu đến cuối.
3. Chọn một hướng nghiên cứu.
4. Giữ hoặc tạo một bảng tóm tắt nhỏ.
5. Viết caption 1-2 câu.
6. Viết một diễn giải 100-150 từ.
7. Dùng sentence frame để viết một câu paper-facing đầu tiên.
8. Ghi một hạn chế hoặc câu hỏi bạn còn băn khoăn.

Các cell đọc CSV và xuất bảng là code mẫu. Ở Tuần 01, bạn chỉ cần chạy và quan sát kết quả.

## Cấu trúc memo bắt buộc

```text
Chosen track:
Small research question:
Data I would need:
Possible table or figure:
Paper-facing sentence:
Limitation:
```

Bạn có thể điền bằng tiếng Việt hoặc English.

Sentence frame gợi ý:

```text
This project asks [small question] because [research reason]. The first dataset would contain [unit of observation], so Python can help by [table/figure/output].
```

## Submission

- Chỉ cần nộp `live_coding.ipynb` đã hoàn thành.

Notebook nên có hướng đã chọn, bảng/summary đã in ra, caption, memo, một câu paper-facing, và ghi chú nguồn/ngày truy cập.

Nhắc lại về caption:

```text
Bảng 1. [Bảng cho thấy điều gì]. Dữ liệu lấy từ dataset giảng dạy do giảng viên tạo cho Tuần 01, truy cập ngày 2026-06-03. N = [4 hướng hoặc 1 hướng đã chọn].
```

## Stretch Task

Mở `interactive_demo.html` và chọn một nút hướng nghiên cứu. Viết 3-4 câu:

- Điều gì thay đổi trên trang?
- Hướng này sau này cần bảng hoặc hình nào?
- Dữ liệu nào sẽ khó thu thập hoặc khó mã hóa?
