...
| Error Category | Score/Severity 1 | Score/Severity 2 | Score/Severity 3 |
|---|---|---|---|
| `word_order` | Word order issue is visible but meaning remains clear. | Word order disrupts the sentence frame and needs guided correction. | Word order makes the response hard to interpret or blocks the target structure. |
| `measure_word` | Wrong classifier but object is clear. | Generic classifier replaces a required specific classifier. | Classifier/noun phrase is unclear enough to affect task completion. |
